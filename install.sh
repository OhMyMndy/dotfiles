...
if command -v vim &>/dev/null && [[ -d /etc/sudoers.d ]]; then
	echo "Defaults editor=$(command -v vim)" | sudo tee /etc/sudoers.d/editor >/dev/null
fi
...
function user_in_group() {
	id -nG "$USER" | tr ' ' '\n' | grep -q "^$1$" && getent group "$1" &>/dev/null
}

groups_to_add=(docker libvirt libvirt-qemu kvm)
