...
function minimal() {
	...
	packages+=(aspell hyphen-en-gb hyphen-nl hunspell aspell-nl hunspell-nl aspell-en hunspell-en-gb hunspell-en-us hunspell-no aspell-no libgspell-1-1 libgtkspell0 python3-hunspell )
	...
	packages+=(redshift-gtk xfce4-clipman-plugin xfce4-terminal xfce4-genmon-plugin xfdashboard chromium-browser seahorse galculator orage ristretto xsel xclip arandr wmctrl xscreensaver flatpak compton catfish rofi xdotool ssh-askpass)
	
	_purge xfce4-appmenu-plugin
	
	_add_repo_or_install_deb 'ppa:hluk/copyq' 'copyq' "https://github.com/hluk/CopyQ/releases/download/v3.10.0/copyq_3.10.0_Debian_10-1_$(cpu_architecture_simple).deb"
